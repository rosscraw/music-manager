...
public class DBConnection {
    ...
    /**
     * Creates the tables for the music management data if they have not already
     * been created.
     * 
     * @throws Exception
     */
    public static void createTables() throws Exception {
        try {
            ...
            // User Table
            PreparedStatement createUserTable = conn.prepareStatement("CREATE TABLE IF NOT EXISTS user("
                    + "id int NOT NULL AUTO_INCREMENT," + " username varchar(255)," + " password varchar(255),"
                    + " role varchar(255)," + " PRIMARY KEY (id)," + " UNIQUE (username))");
            createUserTable.executeUpdate();
            // Singer Table
            PreparedStatement createSingerTable = conn.prepareStatement("CREATE TABLE IF NOT EXISTS singer("
                    + "id int NOT NULL AUTO_INCREMENT," + " name varchar(255)," + " dob int," + " sex varchar(255),"
                    + " company varchar(255)," + " date date," + " PRIMARY KEY (id)," + " UNIQUE (name))");
            createSingerTable.executeUpdate();
            // Album Table
            PreparedStatement createAlbumTable = conn.prepareStatement(
                    "CREATE TABLE IF NOT EXISTS album(" + "id int NOT NULL AUTO_INCREMENT," + " title varchar(255),"
                            + " year int," + " singer varchar(255)," + " company varchar(255)," + " PRIMARY KEY (id),"
                            + " UNIQUE (title)," + " FOREIGN KEY (singer) REFERENCES Singer(name) ON DELETE CASCADE)");
