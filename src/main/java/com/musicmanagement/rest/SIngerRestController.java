...
public class SingerRestController{
    ...
    public List<Singer> search(@PathVariable String search ) {
        List<Singer> match = new ArrayList<Singer>();
        
        if(list().size() > 0) {
            for(int i = 0; i < list().size(); i++) {
                if (list().get(i).getName().toLowerCase().trim().contains(search.toLowerCase().trim())) {
